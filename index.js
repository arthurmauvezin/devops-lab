...
//Creation des données dans les differents objets
...
}); // Pare feu avec erreur 403 si erreur de clée
...
//Lecture des données des differentes routes
...
//Upload des données. On change la valeur d'une ou des données de l'objet.
//Si un ou plusieurs attribut est indéfini, on ne l'ajoute pas dans la requête.
...
//Suppression de(s) donnée(s) des objets
...
// Une simple division, que l'on renomme à l'aide du AS.
// Si la valeur est null, on la remplace par 0 car sinon, problème de type
...
//Relations
//On lis les données de la relation animal dans cage
//Pour le filtre field, on remplace l'ensemble des attributs par celui entré dans l'URL à l'aide de la fonction .replace()
...
// Relation cages dans animal
// Pour le filtre opération, ne pas oublier d'ajouter le nom de la table associé aux attributs que l'on veut afficher
...
// Lire relation food dans animal
app.get('/animals/:id/food', function(req, res) {
    var conditions = ["id", "name", "quantity", "id_animal"];    var id = req.params.id;
...
// Lire relation animal dans food
...
app.get('/animals/:id_animal/food/:id_food', function(req, res) {
    ...
    var conditions = ["id", "name", "quantity", "id_animal"];    var query = "SELECT * FROM animals INNER JOIN food ON animals.id = food.id_animal WHERE animals.id=" + id_animal + " AND food.id=" + id_food;
    if ("fields" in req.query) {
        query = query.replace("*", "food." + req.query["fields"]);
    } else {
        query = query.replace("*", "cages.id, cages.name, cages.description, cages.area");
