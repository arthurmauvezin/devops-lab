//TP1
const express = require('express'); // Charging des librairies
...
app.use( function (req, res, next) {  
    if  ( "key"   in  req.query) {
        ...
        var  query =  "SELECT * FROM users WHERE apikey='"  + key +  "'" ;//apikey : ceciestmonjeton
        db.query(query,  function (err, result, fields) {  
            ...
            if  (result.length >  0 ) { 
                ...
                res.sendStatus(403); // Erreur 403 if key differnet from ceciestmonjeton
            } 
         }); 
    }  
    else  {
...
// FILTRE D'ORDRE
    // Tri by^parameter in url
	if ("sort" in req.query) {
		var sort = req.query["sort"].split(",");
		query += " ORDER BY";
		for (var index in sort) {
			var direction = sort[index].substr(0, 1); //We take out the last element entered
...
// FILTRE DES CHAMPS
    //if fields are in the url
    if  ( "fields"   in  req.query) {
        query = query.replace( "*" , req.query[ "fields" ]); }
    
// FILTRE PAR PAGINATION
    // Limit = nbe of filters we want to show
	if ( "limit" in req.query) {
        
        // Concatenation of the query
